            ...
            <div class="row">
                ...
                <?php
                    ...
                    if ($_SERVER['REQUEST_METHOD'] == 'POST')
                    {
                        $pNo = $_POST['pID'];
                        $emp = $_POST['emp'];
                        $sql = "SELECT * FROM projectgroups WHERE uName='$emp' AND projectID = '$pNo'";
                        $query = $conn->query($sql);
                        if($query->num_rows >= 1)
                        {
                            echo '
                            <p> <b> This employee has already been assigned to this project </b> </p>
                            <form action="project.php" method="Post">
                                <input type="hidden" name="pID" value="'.$pNo.'">
                                
                                <button type="" onClick=""> Return to project </button>
                            </form>

                        ';
                        }
                        else{
                            $sql = "INSERT INTO projectgroups (uName, projectID) VALUES ('$emp','$pNo')";
                            if ($conn->query($sql) === TRUE)
                            {
                                echo '
                                    <p> <b> The employee has been added to the project successfully </b> </p>
                                    <form action="project.php" method="Post">
                                        <input type="hidden" name="pID" value="'.$pNo.'">
